    ...
    public void GravityForce()
    {
        ...
        foreach (Object obj in Object.universe)
        {
            if (obj != this)
            {
                ...
                gSum.x += (trgRot.x / Math.Abs(trgRot.y)) * (Physics.G * this.physics.mass * obj.physics.mass / Vector2.Distance(this.transform.position, obj.transform.position));
                gSum.y += (trgRot.y / Math.Abs(trgRot.y)) * (Physics.G * this.physics.mass * obj.physics.mass / Vector2.Distance(this.transform.position, obj.transform.position));
            }
        }
